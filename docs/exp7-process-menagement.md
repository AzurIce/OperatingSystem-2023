...
在原有的顺序链接应用并按下标获取的基础上，额外存储了所有应用的名称，顺序与下标对应，于是便可以通过应用名获取应用下标，进而获取应用并加载。

### 2. 分析进程标识符、进程控制块是如何设计和实现的；

进程标识符由 `PidAllocator` 顺序分配，释放时加入回收 `Vec` 便于再次使用。

进程控制块中除了之前便有的当前进程的 Trap 上下文、Task 上下文、内存空间等信息外，还额外包含了进程的父子关系，以及 pid。

exec 会加载 elf 替换掉当前进程控制块中的内容。

fork 会复制一份当前的进程控制块及内存空间等内容，并分配一个新 pid，设置父子关系，然后返回。

### 3. 分析任务管理是如何实现的；

任务管理器是一个简单的 FIFO 调度器，有一个 TCB 队列。

`PROCESSOR` 为全局的处理器资源，run_task 会依次从就绪队列中取出任务，设置其状态为 Running，然后将 TCB 转移给 processor，并进行切换任务。

### 4. 分析进程的调度、生成、以及进程资源的回收是如何实现的。

就像之前的多道程序，通过中断 trap 保存上下文，切换上下文，不过现在使用 TCB 来进行。

生成除了最初的 initproc 外都依靠 fork 和 exec 来创建。

进程结束时要进行资源回收，先设置进程状态为 Zombie，然后清理页表等资源，父进程可以通过 waitpid 来收集其信息。

## 三、Git 提交记录

![image-20240103131030054](./assets/image-20240103131030054.png)
